...
NPF_VISIBILITY int npf_vpprintf(npf_putc pc, void* pc_ctx, char const* format, va_list vlist)
        NPF_PRINTF_ATTR(3, 0);
    ...
    while (*cur) {
        ...
        switch (fs.conv_spec) {
            ...
            case NPF_FMT_SPEC_CONV_POINTER: {
                cbuf_len = npf_utoa_rev((npf_uint_t)(uintptr_t)va_arg(args, void*), cbuf, 16,
                                        'a' - 'A');
                need_0x = 'x';
            } break;
