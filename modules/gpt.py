...
def modify_step1(part_list):
    ...
    partition_n['name'] = "boot_a_tmp".encode("utf-16le") + b"\x00\x00"
    ...
    partition_n['end'] = partition_n['start'] + 0x37000 - 1
    partition_n['name'] = "boot_b_tmp".encode("utf-16le") + b"\x00\x00"
    part_list_n.append(partition_n)
...
def modify_step2(part_list):
    ...
    part_list_n = part_list.copy()
    partition = get_part_by_name(part_list_n, "boot_a")
    if partition:
        partition["name"] = "boot_a_x".encode("utf-16le") + b"\x00\x00"

    partition = get_part_by_name(part_list_n, "boot_b")
    if partition:
        partition["name"] = "boot_b_x".encode("utf-16le") + b"\x00\x00"

    partition = get_part_by_name(part_list_n, "boot_a_tmp")
    if partition:
        partition["name"] = "boot_a".encode("utf-16le") + b"\x00\x00"

    partition = get_part_by_name(part_list_n, "boot_b_tmp")
    if partition:
        partition["name"] = "boot_b".encode("utf-16le") + b"\x00\x00"
...
def unpatch(gpt_header, part_list):
    ...
    partition = get_part_by_name(part_list_n, "boot_a_x")
    if partition:
        partition["name"] = "boot_a".encode("utf-16le") + b"\x00\x00"

    partition = get_part_by_name(part_list_n, "boot_b_x")
    if partition:
        partition["name"] = "boot_b".encode("utf-16le") + b"\x00\x00"
    return part_list_n
