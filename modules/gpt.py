...
def get_part_table(gpt_data, gpt_header):
    if gpt_header["part_lba"] != 2:
        start_lba = -(
            ((gpt_header["part_size"] * gpt_header["part_num"]) // BLOCK_SIZE) + 1
        )
    else:
...
def parse_part_table(part_table, gpt_header):
    ...
    assert (
        part_crc32 == gpt_header["part_crc32"]
    ...
    for partition_num in range(0, gpt_header["part_num"]):
        partition = parse_partition(
            part_table, gpt_header["part_size"] * partition_num, gpt_header["part_size"]
        )
        ...
        log(
            "{:>5}  {:>15}  {:>15}  {:<12s}  {:<15} ".format(
                partition_num + 1, partition["start"], partition["end"], size, name
            )
        )
