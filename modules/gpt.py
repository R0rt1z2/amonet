...
def calc_header_crc32(sector, header_size):
    return zlib.crc32(
        sector[:0x10] + b'\x00\x00\x00\x00' + sector[0x10 + 4 : header_size]
    )
...
def parse_header(gpt_data, lba):
    ...
    gpt_header_t = namedtuple(
        'gpt_header',
        'signature version header_size header_crc32 reserved this_lba other_lba first_lba last_lba guid part_lba part_num part_size part_crc32',
    )
    gpt_header = gpt_header_t(
        *struct.unpack(HEADER_STRUCT, sector[:512])
    )._asdict()
    ...
    assert header_crc32 == gpt_header['header_crc32'], (
        'GPT header is corrupt, expected CRC32 %.8X but got %.8X'
        % (
            header_crc32,
    ...
    log(
        ...
        % str(uuid.UUID(bytes_le=gpt_header['guid'])).upper()
    )
    log('Partition table holds up to %d entries' % gpt_header['part_num'])
    log(
        'This partition table begins at sector %d and ends at sector %d'
        % (
            gpt_header['part_lba'],
            gpt_header['part_lba']
            - 1
            + (gpt_header['part_num'] * gpt_header['part_size'] / BLOCK_SIZE),
        )
    )
    log(
        'First usable sector is %d, last usable sector is %d'
        % (gpt_header['first_lba'], gpt_header['last_lba'])
    )
...
def parse_partition(part_table, offset, size):
    partition_t = namedtuple(
        'partition', 'type_guid guid start end attrib name'
    )
    partition = partition_t(
        *struct.unpack(PART_STRUCT, part_table[offset : offset + size])
    )._asdict()
    return partition
...
def parse_part_table(part_table, gpt_header):
    ...
    part_table += b'\x00' * (
        (gpt_header['part_num'] * gpt_header['part_size']) - len(part_table)
    )

    part_crc32 = zlib.crc32(part_table)
    assert part_crc32 == gpt_header['part_crc32'], (
        'Partition table is corrupt, expected CRC32 %.8X but got %.8X'
        % (
            part_crc32,
            gpt_header['part_crc32'],
        )
    )

    log(
        '{:<5}  {:>15}  {:>15}  {:<12}  {:<15} '.format(
    ...
    for partition_num in range(0, gpt_header['part_num']):
        partition = parse_partition(
            part_table,
            gpt_header['part_size'] * partition_num,
            gpt_header['part_size'],
        )
...
def parse_gpt_compat(gpt_data):
    ...
    parts = dict()
    for part in part_list:
        parts[part['name'].decode('utf-16le').strip('\x00')] = (
            part['start'],
...
def generate_gpt(gpt_header, part_list):
    ...
    primary = (
        ...
        + create_header(
            PRIMARY_GPT_LBA,
            gpt_header['last_lba'] + part_blocks,
            gpt_header['last_lba'],
            gpt_header['guid'],
            2,
            part_crc32,
    ...
    backup = part_table + create_header(
        gpt_header['last_lba'] + part_blocks,
        1,
        gpt_header['last_lba'],
        gpt_header['guid'],
...
def modify_step1(part_list):
    ...
    assert partition['name'].decode('utf-16le').rstrip('\x00') == 'userdata', (
    ...
    assert (
        ...
        and not get_part_by_name(part_list_n, 'boot_tmp')
...
def unpatch(gpt_header, part_list):
    ...
    assert userdata_index is not None, (
        'No userdata partition found, refusing to unpatch'
    )
...
def main():
    if len(sys.argv) == 2:
        ...
        print(
            'Usage: ' + sys.argv[0] + ' [ print | patch | unpatch ] <filename>'
        )
        sys.exit()
    ...
    if cmd == 'patch':
        ...
        primary_unpatched, backup_unpatched = generate_gpt(
            gpt_header, part_list_unpatched
        )
